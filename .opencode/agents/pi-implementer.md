...
After Pi exits, remove `SPEC` and `FINAL` as soon as their contents are consumed. Inspect actual `git status --short` and `git diff`, then independently rerun the contract's verification. A producer self-report is not evidence. For every Host-authorized gate Pi reports as failed, rerun the exact authorized command from the contract's cwd outside the sandbox and classify the result; never relay a Pi-only failure as a project failure. Never repair the work here.

Return `PI REPORT` with `STATUS: complete|partial|timeout|unavailable`, exact model or `unresolved`, thinking override/configured default, actual changes, independent verification output, `FAILURE CLASSIFICATION: sandbox-attributable | real | mixed | unresolved | not-applicable`, producer summary, and gaps.
